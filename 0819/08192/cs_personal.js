            ...
            .then((doc) => {
              if (doc.exists) {
                ...
                if (doc.data().level > 1) {
                  ...
                  const hidden1 = document.querySelector("#personal-btn");
...
function dbtable() {
  ...
  db.collection("QNA")
    ...
    .then((snapshot) => {
      ...
      for (
        ...
        const template = `
          <tr>
              ...
              <td><a class="cs_title" href="./cs_edit_personal.html?id=${
                snapshot.docs[i].id
              }">
      ...
      $(".pagenation").append("");
      /*페이지 네비게이션 처리 위치*/
      const pagePath = "cs_personal.html";
...
//검색하기 버튼에 온클릭으로 연결하게해둠 파베용이라 한계있음
function searchList() {
  const db = firebase.firestore();
